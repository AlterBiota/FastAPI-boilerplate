...
async def login_for_access_token(
    ...
    access_token = await create_access_token(data={"sub": user["uuid"],
                                                    "client": user["id"],
                                                    "role": user["role"]}, expires_delta=access_token_expires)

    refresh_token = await create_refresh_token(data={"sub": user["uuid"],
                                                    "client": user["id"],
                                                    "role": user["role"]})
...
async def refresh_access_token(request: Request, db: AsyncSession = Depends(async_get_db)) -> dict[str, str]:
    ...
    new_access_token = await create_access_token(data={"sub": user_data.user_uuid,
                                                        "client": user_data.client_id,
                                                        "role": user_data.role})

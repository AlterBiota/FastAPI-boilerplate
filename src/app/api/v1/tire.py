...
from ...schemas.client import TierCreate, TierCreateInternal, TierRead, TierUpdate
from ..dependencies import get_current_superuser

router = APIRouter(tags=["client"])


@router.post("/client", dependencies=[Depends(get_current_superuser)], status_code=201)
async def write_client(
    ...
    if db_tier:
        raise DuplicateValueException("Client Name not available")
...
@router.get("/clients", response_model=PaginatedListResponse[TierRead], dependencies=[Depends(get_current_superuser)])
...
#Requires Client Admin or Superuser
@router.patch("/client/{name}", dependencies=[Depends(get_current_superuser)])
